...
TEST(micro_exch_sparse, scalar_A_COO_CSR_comparison) {
    ...
    af::array m = af::mean(af::matmul(exch_COO.matr - exch_CSR.matr, af::flat(af::constant(1., nx, ny, nz, 3, f64))));
    // af::print("", m);
...
TEST(micro_exch_sparse, array_A_COO_CSR_comparison) {
    ...
    auto exch_CSR = SparseExchangeField(af::constant(A, nx, ny, nz, f64), mesh, true, false);
    af::array m = af::mean(af::matmul(exch_COO.matr - exch_CSR.matr, af::flat(af::constant(1., nx, ny, nz, 3, f64))));
    // af::print("", m);
    EXPECT_EQ(m.scalar<double>(), 0);
}

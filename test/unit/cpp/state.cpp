...
TEST(State, integral_nonequimesh_dz_as_gauss_sum) {
    const float dx=1e-9, dy=2e-9, dz=3e-9;
    ...
    af::array m = af::constant(0.0, mesh.dims, f32);
    for(int i=0; i<nz; i++){
        if( i%2 == 0){
    ...
    float positive_sum =   nz/2 * (1 + (nz-1))/2;// 1 + 3 + 5 ... + 9
    float negative_sum = - nz/2 * (2 + nz)/2;// 2 + 4 + 6 ... + 10
    float result = dx * nx * dy * ny * dz * (positive_sum + negative_sum);
...
TEST(State, vtkIO_vtrWriteReadTest) {
    ...
    State read_state(NonequispacedMesh(0, 0, 0, 0, {0}), 4e5, af::constant(0, 1, f32), false, true);

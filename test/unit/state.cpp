...
TEST(State, mean_m) {
    ...
    // vec_as_array method:
    auto m = [dims = mesh.dims_scalar](std::array<double, 3> vec) {
    ...
    // Ms.field
    auto Ms_field = af::constant(Ms, mesh.dims_scalar, f64);
    test({mesh, Ms_field, m({1, 0, 0})}, {1, 0, 0});
    test({mesh, Ms_field, m({0, 1, 0})}, {0, 1, 0});
    test({mesh, Ms_field, m({0, 0, 1})}, {0, 0, 1});

    // Ms.field with zeros
    auto Ms_field_with_zeros = af::constant(0, mesh.dims_scalar, f64);

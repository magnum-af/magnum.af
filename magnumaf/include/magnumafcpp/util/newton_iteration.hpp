...
/// \param EpsilonFactor used for adaptively calculating epsilon \f$ eps =
/// EpsilonFactor * |x_n - x_{n-1}|\f$ for the approximation of the derivative:
/// \f$ f'(x) = \frac{f(x+eps) - f(x)}{eps} \f$
...
class NewtonIteration {
  ...
  private:
    // calculate f(x) and df(x) and return as std::pair <double, double> {f(x),
    // df(x)}

...
ZeroCrossing::ZeroCrossing(std::function<double(double)> f, double precision, int max_runs, double ix_min,
    ...
    : f(std::move(f)), precision(precision), max_runs(max_runs), ix_min(ix_min), ix_max(ix_max), ix_n(ix_n),
      verbose(verbose) {}
...
std::array<double, 4> ZeroCrossing::run_loop() {
    ...
    for (int i = 0; i < ix_n + 1; i++) {
        const double x = ix_min + static_cast<double>(i) / static_cast<double>(ix_n) * (ix_max - ix_min);
        const double fx = f(x);
        if (verbose > 2) {

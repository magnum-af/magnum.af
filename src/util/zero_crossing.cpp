...
ZeroCrossing::ZeroCrossing(std::function<double(double)> f, double precision, int max_runs, double ix_min, double ix_max, int ix_n, int verbose) : f(f), precision(precision), max_runs(max_runs), ix_min(ix_min), ix_max(ix_max), ix_n(ix_n), verbose(verbose)
{
}
...
std::pair<double, double> ZeroCrossing::calc_x_and_f()
{
    ...
    for (int i = 0; i < max_runs; i++)
    {
        ...
        {
            if (verbose)
                std::cout << "No crossing found in given interval. Setting larger x range by a factor of 10" << std::endl;
        ...
        if (verbose > 1)
            std::cout << "i = " << i << ", ix_min = " << ix_min << ", ix_max = " << ix_max << std::endl
                      << std::endl;
    }

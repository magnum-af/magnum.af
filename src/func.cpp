...
std::array<double, 3> spacial_mean_in_region(const af::array& vectorfield,
                                             const af::array& region) {
    if (vectorfield.dims(3) != 3 or region.dims(3) != 1) {
        printf("\33[1;31mWarning:\33[0m spacial_mean_in_region: dimensions do "
               "not match, returning [-1, -1, -1]\n");
        return {-1, -1, -1};
    } else {
        af::array zero_if_input_is_zero_else_one = !af::iszero(region);
        const unsigned number_of_nonzero_elements =
            af::sum(af::sum(af::sum(zero_if_input_is_zero_else_one, 0), 1), 2)
                .scalar<unsigned>();
        af::array considered_values =
            vectorfield * af::tile(zero_if_input_is_zero_else_one, 1, 1, 1, 3);
        af::array sum_considered_values =
            af::sum(af::sum(af::sum(considered_values, 0), 1), 2);
        af::array mean_considered_values =
            sum_considered_values / number_of_nonzero_elements;
...
std::array<double, 3> spacial_mean_in_region(long int vectorfield,
                                             long int region) {
    return spacial_mean_in_region(*(new af::array(*((void**)vectorfield))),
                                  *(new af::array(*((void**)region))));
}
...
af::array cross4(const af::array& a, const af::array& b) {
    ...
    c(af::span, af::span, af::span, 0) =
        a(af::span, af::span, af::span, 1) *
            b(af::span, af::span, af::span, 2) -
        a(af::span, af::span, af::span, 2) * b(af::span, af::span, af::span, 1);
    c(af::span, af::span, af::span, 1) =
        a(af::span, af::span, af::span, 2) *
            b(af::span, af::span, af::span, 0) -
        a(af::span, af::span, af::span, 0) * b(af::span, af::span, af::span, 2);
    c(af::span, af::span, af::span, 2) =
        a(af::span, af::span, af::span, 0) *
            b(af::span, af::span, af::span, 1) -
...
af::array renormalize_handle_zero_values(const af::array& a) {
    ...
    // TODO for (af::array& a) only: return replace(a/tile(sqrt(sum(a*a, 3)), 1,
    // 1, 1, 3), a!=0., 0.);
}
...
double meani(const af::array& a, const int i) {
    ...
    norm_host = mean(mean(mean(a(af::span, af::span, af::span, i), 0), 1), 2)
                    .host<double>();
...
double mean_abs_diff(const af::array& a, const af::array& b) {
    return afvalue(
        af::mean(af::mean(af::mean(af::mean(af::abs(a - b), 0), 1), 2), 3));
}
...
double max_abs_diff(const af::array& a, const af::array& b) {
    return afvalue(
        af::max(af::max(af::max(af::max(af::abs(a - b), 0), 1), 2), 3));
}
...
bool abs_diff_lt_precision(af::array first, af::array second, double precision,
                           bool verbose) {
    unsigned int zero_if_equal = afvalue_u32(af::sum(
        af::sum(af::sum(af::sum(!(af::abs(first - second) < precision), 0), 1),
                2),
        3));
    if (verbose) {
        if (zero_if_equal == 0)
            ...
            std::cout << "\33[1;31mError!\33[0m " << zero_if_equal << " out of "
                      << first.elements()
                      << " absolute values of element-wise differences are "
                         "above precision of "
                      << precision << std::endl;
...
/// otherwise.
bool rel_diff_lt_precision(af::array first, af::array second, double precision,
                           bool verbose) {
    af::array temp = af::abs(2 * (first - second) / (first + second));
    af::replace(
        temp, first != 0 || second != 0,
    ...
    unsigned int zero_if_equal = afvalue_u32(
        af::sum(af::sum(af::sum(af::sum(!(temp < precision), 0), 1), 2), 3));
    if (verbose) {
        if (zero_if_equal == 0)
            std::cout << "\33[1;32mSucess:\33[0m All " << first.elements()
...
double abs_diff_upperbound(const af::array& a, const af::array& b, bool verbose,
                           double start_precision, double factor1,
                           double factor2) {
...
double rel_diff_upperbound(const af::array& a, const af::array& b, bool verbose,
                           double start_precision, double factor1,
                           double factor2) {
    double prec = start_precision;
    double prec_prev = prec;
...
std::pair<int, int> util::k2ij(const int k, const int n) {
    const int i = n - 1 -
                  static_cast<int>(std::floor(
                      std::sqrt(-8 * k + 4 * n * (n + 1) - 7) / 2.0 - 0.5));
...
unsigned int util::stride(const unsigned int i, const unsigned int j,
                          const unsigned int ni) {
    return i + ni * j;
}
unsigned int util::stride(const unsigned int i, const unsigned int j,
                          const unsigned int k, const unsigned int ni,

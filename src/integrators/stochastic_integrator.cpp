...
void Stochastic_Integrator::step(State &state)
{ //TODO remove dt as parameter here, inconsistency between Heun/SemiHeun
...
Stochastic_Integrator::Stochastic_Integrator(double alpha, double T, double dt, State state, std::vector<std::shared_ptr<LLGTerm>> Fieldterms_in, std::string smode) : alpha(alpha), T(T), dt(dt), Fieldterms(Fieldterms_in), m_prev(state.m)
{
    const double D = (this->alpha * constants::kb * this->T) / (constants::gamma * constants::mu0 * state.Ms * state.mesh.V);
    ...
    {
        std::cout << "ERROR: Constructor Stochastic_Ingetrator: Integrator Mode not recognized" << std::endl;
    }
}

...
array AtomisticDipoleDipoleField::h(const State &state)
{
    ...
    // Pointwise product
    hfft(span, span, span, 0) = Nfft(span, span, span, 0) * mfft(span, span, span, 0) + Nfft(span, span, span, 1) * mfft(span, span, span, 1) + Nfft(span, span, span, 2) * mfft(span, span, span, 2);
    hfft(span, span, span, 1) = Nfft(span, span, span, 1) * mfft(span, span, span, 0) + Nfft(span, span, span, 3) * mfft(span, span, span, 1) + Nfft(span, span, span, 4) * mfft(span, span, span, 2);
    hfft(span, span, span, 2) = Nfft(span, span, span, 2) * mfft(span, span, span, 0) + Nfft(span, span, span, 4) * mfft(span, span, span, 1) + Nfft(span, span, span, 5) * mfft(span, span, span, 2);
    ...
    for (int i0 = 0; i0 < n0_exp; i0++)
    {
        ...
        for (int i1 = 0; i1 < n1_exp; i1++)
        {
            ...
            for (int i2 = 0; i2 < n2_exp; i2++)
            {
                ...
                if (r == 0.)
                { //TODO repsace with if (j0 == 0 && j1 == 0 && j2 == 0)

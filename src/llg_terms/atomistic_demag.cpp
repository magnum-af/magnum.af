...
double AtomisticDipoleDipoleField::E(const State& state) {
    ...
    // return -state.Ms/2 * afvalue(sum(sum(sum(sum(h(state)*state.m, 0), 1),
    // 2), 3));
}
...
array N_atomistic(int n0_exp, int n1_exp, int n2_exp, double dx, double dy,
                  double dz);
...
array AtomisticDipoleDipoleField::h(const State& state) {
    ...
    if (state.mesh.n2_exp == 1) {
        ...
        mfft = fftR2C<3>(state.m, dim4(state.mesh.n0_exp, state.mesh.n1_exp,
                                       state.mesh.n2_exp));
    }

    array hfft = array(state.mesh.n0_exp / 2 + 1, state.mesh.n1_exp,
                       state.mesh.n2_exp, 3, c64);
    // Pointwise product
    hfft(span, span, span, 0) =
        Nfft(span, span, span, 0) * mfft(span, span, span, 0) +
        Nfft(span, span, span, 1) * mfft(span, span, span, 1) +
        Nfft(span, span, span, 2) * mfft(span, span, span, 2);
    hfft(span, span, span, 1) =
        Nfft(span, span, span, 1) * mfft(span, span, span, 0) +
        Nfft(span, span, span, 3) * mfft(span, span, span, 1) +
        Nfft(span, span, span, 4) * mfft(span, span, span, 2);
    hfft(span, span, span, 2) =
        Nfft(span, span, span, 2) * mfft(span, span, span, 0) +
    ...
    if (state.mesh.n2_exp == 1) {
        ...
        return state.Ms *
               h_field(seq(0, state.mesh.n0_exp / 2 - 1),
                       seq(0, state.mesh.n1_exp / 2 -
                                  1)); // TODO consider p density, then we have
                                       // to multip at m before fft
    } else {
        h_field = fftC2R<3>(hfft);
        if (state.afsync)
            af::sync();
        cpu_time += timer::stop(timer_demagsolve);
        return state.Ms * h_field(seq(0, state.mesh.n0_exp / 2 - 1),
                                  seq(0, state.mesh.n1_exp / 2 - 1),
                                  seq(0, state.mesh.n2_exp / 2 - 1), span);
    }
}

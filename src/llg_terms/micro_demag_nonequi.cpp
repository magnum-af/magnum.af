...
af::array NonEquiDemagField::h(const State &state)
{
    ...
    for (int i_source = 0; i_source < state.nonequimesh.nz; i_source++)
    {
        for (int i_target = 0; i_target < state.nonequimesh.nz; i_target++)
        {
            ...
            { // This reflects the data structure of newell_nonequi::N_ptr. "<=" choosen by definition, could also be ">(=)" or just "<" when reconsidering N_ptr
            ...
            hfft(af::span, af::span, i_target, 0) += nfft(af::span, af::span, af::span, 0) * mfft(af::span, af::span, i_source, 0) + nfft(af::span, af::span, af::span, 1) * mfft(af::span, af::span, i_source, 1) + nfft(af::span, af::span, af::span, 2) * mfft(af::span, af::span, i_source, 2);
            hfft(af::span, af::span, i_target, 1) += nfft(af::span, af::span, af::span, 1) * mfft(af::span, af::span, i_source, 0) + nfft(af::span, af::span, af::span, 3) * mfft(af::span, af::span, i_source, 1) + nfft(af::span, af::span, af::span, 4) * mfft(af::span, af::span, i_source, 2);
            hfft(af::span, af::span, i_target, 2) += nfft(af::span, af::span, af::span, 2) * mfft(af::span, af::span, i_source, 0) + nfft(af::span, af::span, af::span, 4) * mfft(af::span, af::span, i_source, 1) + nfft(af::span, af::span, af::span, 5) * mfft(af::span, af::span, i_source, 2);
        }
    }
...
double Nxx(const double x, const double y, const double z, const double dx, const double dy, const double dz, const double dX, const double dY, const double dZ)
{
    ...
    return -1. / (4.0 * M_PI) * (F0(x, y, z, dy, dY, dz, dZ) - F0(x - dx, y, z, dy, dY, dz, dZ) - F0(x + dX, y, z, dy, dY, dz, dZ) + F0(x - dx + dX, y, z, dy, dY, dz, dZ));
}
...
double Nxy(const double x, const double y, const double z, const double dx, const double dy, const double dz, const double dX, const double dY, const double dZ)
{
    ...
    //const double tau = dX * dY * dZ;// Defining dX, dY, dZ as target cuboid (one could alternatively choose dx, dy, dz with implications on x, y, z)
    //return -1./(4.0 * M_PI * tau) * (
    return -1. / (4.0 * M_PI) * (G0(x, y, z, dy, dY, dz, dZ) - G0(x - dx, y, z, dy, dY, dz, dZ) - G0(x + dX, y, z, dy, dY, dz, dZ) + G0(x - dx + dX, y, z, dy, dY, dz, dZ));
}
...
public:
    NonequiLoopInfo() {}
    NonequiLoopInfo(int ix_start, int ix_end, int n0_exp, int n1_exp, int n2, double dx, double dy) : ix_start(ix_start), ix_end(ix_end), n0_exp(n0_exp), n1_exp(n1_exp), n2(n2), dx(dx), dy(dy) {}
    int ix_start;
    int ix_end;
    int n0_exp;
    int n1_exp;

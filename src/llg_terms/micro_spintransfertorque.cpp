...
af::array SpinTransferTorqueField::h(const State& state){
    ...
    return - j_e * constants::hbar / (2. * constants::e * constants::mu0 * state.material.ms) * (nu_dampinglike * cross4(state.m, polarization_field.array) + nu_fieldlike * polarization_field.array);
}

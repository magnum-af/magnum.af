...
NonequiExchangeField::NonequiExchangeField (float A_exchange, NonequispacedMesh mesh, bool verbose) : matr(calc_CSR_matrix(A_exchange, mesh, verbose))
{
}
...
float NonequiExchangeField::E(const State& state){
    return - constants::mu0/2. * state.integral_nonequimesh(h(state) * state.m);
}


float NonequiExchangeField::E(const State& state, const af::array& h){
...
af::array NonequiExchangeField::calc_CSR_matrix(const float A_exchange, const NonequispacedMesh& mesh, const bool verbose){
    printf("%s NonequiExchangeField::calc_CSR_matrix unit testing not finished!\n", Warning()); fflush(stdout);
    af::timer t;
    if(verbose) af::timer::start();

    std::vector<float> h;// spacings between discretization points h = (dz[n] + dz[n+1])/2
            ...
            for (int i0 = 0; i0 < mesh.nx; i0++){
              ...
              if(ind==id) {
                ...
                if (i2 == 0 && mesh.nz > 1 ){//TODO check
                  ...
                  float h_divisor = h.at(i2) * h.at(i2-1) * (1. + h.at(i2)/h.at(i2-1));
                  // f_{i-1} term
          ...
          for (int i1 = 0; i1 < mesh.ny; i1++){
            for (int i0 = 0; i0 < mesh.nx; i0++){
              ...
              if(ind==id) {
                ...
                if(i0 == 0 && mesh.nx > 1){
                    float A_i = a_host[util::stride(i0, i1, i2, mesh.nx, mesh.ny)];
                    float A_i_p = a_host[util::stride(i0+1, i1, i2, mesh.nx, mesh.ny)];
                    if (A_i != 0){
                    ...
                    float A_i = a_host[util::stride(i0, i1, i2, mesh.nx, mesh.ny)];
                    float A_i_m = a_host[util::stride(i0-1, i1, i2, mesh.nx, mesh.ny)];
                    if (A_i != 0){
                ...
                if(i1 == 0 && mesh.ny > 1 ){
                    float A_i = a_host[util::stride(i0, i1, i2, mesh.nx, mesh.ny)];
                    float A_i_p = a_host[util::stride(i0, i1+1, i2, mesh.nx, mesh.ny)];
                    if (A_i != 0){
                        CSR_values.push_back( (2.* A_i)/(constants::mu0 * pow(mesh.dx, 2)) * 2.* A_i_p/(A_i_p + A_i));
                        CSR_JA.push_back( findex( i0, i1+1, i2, im, mesh ) );
                        csr_ia++;
                    ...
                    float A_i = a_host[util::stride(i0, i1, i2, mesh.nx, mesh.ny)];
                    float A_i_m = a_host[util::stride(i0, i1-1, i2, mesh.nx, mesh.ny)];
                    if (A_i != 0){
                        CSR_values.push_back( (2.* A_i)/(constants::mu0 * pow(mesh.dx, 2)) * 2.* A_i_m/(A_i_m + A_i));
                        CSR_JA.push_back( findex( i0, i1-1, i2, im, mesh ) );
                        csr_ia++;
                ...
                if (i2 == 0 && mesh.nz > 1 ){
                    float A_i = a_host[util::stride(i0, i1, i2, mesh.nx, mesh.ny)];
                    float A_i_p = a_host[util::stride(i0, i1, i2+1, mesh.nx, mesh.ny)];
                    if (A_i != 0){
                    ...
                    float A_i = a_host[util::stride(i0, i1, i2, mesh.nx, mesh.ny)];
                    float A_i_m = a_host[util::stride(i0, i1, i2-1, mesh.nx, mesh.ny)];
                    if (A_i != 0){
                    ...
                    const float A_i = a_host[util::stride(i0, i1, i2, mesh.nx, mesh.ny)];
                    const float A_i_m = a_host[util::stride(i0, i1, i2-1, mesh.nx, mesh.ny)];
                    const float h_divisor = h.at(i2) * h.at(i2-1) * (1. + h.at(i2)/h.at(i2-1));
                    if (A_i_m != 0){
                        CSR_values.push_back( 2.* A_i/constants::mu0 * (2. * h.at(i2)/h.at(i2-1))/h_divisor * 2.* A_i_m/(A_i_m + A_i));
                        CSR_JA.push_back( findex( i0, i1, i2-1, im, mesh ) );
                        csr_ia++;
                    }
                    const float A_i_p = a_host[util::stride(i0, i1, i2+1, mesh.nx, mesh.ny)];
                    if (A_i_p != 0){

...
NonequiUniaxialAnisotropyField::NonequiUniaxialAnisotropyField(
    af::array Ku1_field, std::array<double, 3> Ku1_axis)
    : Ku1_field(Ku1_field), Ku1_axis(get_normalized_vector(Ku1_axis)) {}

NonequiUniaxialAnisotropyField::NonequiUniaxialAnisotropyField(
    af::array Ku1_field, af::array Ku1_axis_field)
    : Ku1_field(Ku1_field), Ku1_axis_field(Ku1_axis_field) {}

NonequiUniaxialAnisotropyField::NonequiUniaxialAnisotropyField(
    double Ku1, long int Ku1_axis_field_ptr)
    : Ku1(Ku1),
      Ku1_axis_field(*(new af::array(*((void**)Ku1_axis_field_ptr)))) {}

NonequiUniaxialAnisotropyField::NonequiUniaxialAnisotropyField(
    long int Ku1_field_ptr, long int Ku1_axis_field_ptr)
    : Ku1_field(*(new af::array(*((void**)Ku1_field_ptr)))),
      Ku1_axis_field(*(new af::array(*((void**)Ku1_axis_field_ptr)))) {}

// For wrapping only
NonequiUniaxialAnisotropyField::NonequiUniaxialAnisotropyField(
    double Ku1, double Ku1_axis_0, double Ku1_axis_1, double Ku1_axis_2)
    : Ku1(Ku1), Ku1_axis(get_normalized_vector(std::array<double, 3>{
                    Ku1_axis_0, Ku1_axis_1, Ku1_axis_2})) {}

// For wrapping only
NonequiUniaxialAnisotropyField::NonequiUniaxialAnisotropyField(
    long int Ku1_field_ptr, double Ku1_axis_0, double Ku1_axis_1,
    double Ku1_axis_2)
    : Ku1_field(*(new af::array(*((void**)Ku1_field_ptr)))),
      Ku1_axis(get_normalized_vector(
...
af::array NonequiUniaxialAnisotropyField::calc_heff(const State& state) {
    ...
    if (state.Ms_field.isempty() && Ku1_field.isempty()) {
        ...
        af::array result =
            2. * Ku1 / (constants::mu0 * state.Ms_field) * (eu * anisotropy);
        ...
        af::array result = 2. * Ku1_field / (constants::mu0 * state.Ms_field) *
                           (eu * anisotropy);
        af::replace(result, !af::iszero(state.Ms_field),
                    0); // Replacing all resulting NaN with 0
        return result;
...
std::array<double, 3> NonequiUniaxialAnisotropyField::get_normalized_vector(

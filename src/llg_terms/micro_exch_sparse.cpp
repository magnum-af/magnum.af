...
array ExchSolverSparse::h(const State& state){
...
//}
//
//double ExchSolverSparse::E(const State& state, const af::array& h){//TODO this should use h_width_edges, check if h instead of h makes difference
//  return -param.mu0/2. * param.ms * afvalue(sum(sum(sum(sum(h * state.m,0),1),2),3)) * mesh.dx * mesh.dy * mesh.dz;
//}
//

...
double LBFGS_Minimizer::Minimize(State &state)
{
    ...
    do
    {
        ...
        y = grad - grad_old;
        double ys = mydot(y, s);
        if (ys <= eps2 * mynorm(y) * mynorm(s))
        { // Dennis, Schnabel 9.4.1
            if (this->verbose > 2)
            {
...
int LBFGS_Minimizer::cvsrch(State &state, const af::array &wa, double &f, af::array &g, double &stp, const af::array &s, const double tolf)
{
    ...
    while (true)
    {
        ...
        if (
            (brackt && ((stp <= stmin) | (stp >= stmax))) | (nfev >= maxfev - 1) | (infoc == 0) | (brackt & (stmax - stmin <= xtol * stmax)))
        {
            stp = stx;
            if (this->verbose > 0)
        ...
        if (stage1 & (f <= fx) & (not((f <= ftest2) && (ft * dginit >= dg))))
        { // not wolfe 1 --> not approx wolfe 1

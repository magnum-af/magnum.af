...
String::String(State state, std::vector<State> inputimages, int n_interp, double dt, LLGIntegrator llg)
    ...
    for (int i = 0; i < n_interp; i++) {
        ...
        if (state.Ms_field.isempty()) {
            images.push_back(State(state.mesh, state.Ms, af::constant(std::sqrt(1 / 3), state.mesh.dims, f64)));
        } else {
            images.push_back(State(state.mesh, state.Ms_field, af::constant(std::sqrt(1 / 3), state.mesh.dims, f64)));
        }
    }
    for (int i = 0; i < n_interp; i++) {
        int j = 0;
        ...
        if (j < n_interp - 1) {
            images[i].m =
                inputimages[j].m + (x_interp[i] - x[j]) * (inputimages[j + 1].m - inputimages[j].m) / (x[j + 1] - x[j]);
        } else {
            std::cout << "Warning: x_interp[j=" << j << "] exceedes x range, y_interp[j] value set to y[j]"
                      << std::endl;
...
void write_plotfile(const std::string filepath) {
    ...
    stream << "set title 'Energy over image ID for each iteration'" << std::endl;
    ...
    stream << "p 'E_curves.dat' u 2:3:1 w l palette t 'E'" << std::endl << std::endl;
    ...
    stream << "p 'dE_over_iteration.dat' u 1:2 w l notitle" << std::endl << std::endl;
...
double String::run(const std::string filepath, const double string_abort_rel_diff, const double string_abort_abs_diff,
    ...
    for (int i = 0; i < string_steps; i++) {
        ...
        for (unsigned j = 0; j < this->E.size(); ++j) {
            stream_E_curves << i << " " << j << " " << this->E[j] - this->E[0] << " " << this->E[j] - this->E[-1] << " "
                            << this->E[j] << std::endl;
        }
        stream_E_curves << i << "\n\n" << std::endl;
        ...
        stream_steps << i << "\t" << std::setw(18) << *max - this->E[0] << "\t" << rel_diff << "\t" << abs_diff
                     << std::endl;
    }
    ...
    for (unsigned i = 0; i < this->images.size(); i++) {
        printf("E[%d]=%e\n", i, E[i]);
        myfileE << i << "\t" << this->E[i] << "\t" << this->E[i] - this->E[0] << "\t" << this->E[i] - this->E[-1]
                << std::endl;
        std::string name = filepath;

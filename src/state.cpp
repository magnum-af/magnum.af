...
void State::set_Ms_field_if_m_minvalnorm_is_zero(const af::array &m, af::array &Ms_field)
{
    ...
    if (minval(vecnorm(m)) == 0)
    {
        if (verbose)
        {
            printf("%s in state.cpp: initial m has values with zero norm, building Ms_field array\n", Info());
        }
        af::array nzero = !af::iszero(vecnorm(m));
        n_cells_ = afvalue_u32(af::sum(af::sum(af::sum(nzero, 0), 1), 2));
        if (Ms == 0)
            printf("Wraning: State::set_Ms_field: State.Ms is used but set to zero. It appears that you are using a legacy constuctor. Please pass Ms in constructor!\n");
...
{ //allowed norm is 1 or 0 (for no Ms_field)
...
State::State(Mesh mesh, af::array Ms_field, af::array m, bool verbose, bool mute_warning) : mesh(mesh), Ms_field(Ms_field.dims(3) == 1 ? af::tile(Ms_field, 1, 1, 1, 3) : Ms_field), m(m), verbose(verbose), mute_warning(mute_warning)
{
    if(Ms_field.dims(3) == 3){printf("%s State: You are using legacy dimension [nx, ny, nz, 3] for Ms, please now use scalar field dimensions [nx, ny, nz, 1].\n", Warning());}
...
State::State(NonequispacedMesh nonequimesh, double Ms, af::array m, bool verbose, bool mute_warning) : nonequimesh(nonequimesh), Ms(Ms), m(m), verbose(verbose), mute_warning(mute_warning)
{
    check_m_norm();
    set_Ms_field_if_m_minvalnorm_is_zero(this->m, this->Ms_field);
}

State::State(NonequispacedMesh nonequimesh, af::array Ms_field, af::array m, bool verbose, bool mute_warning) : nonequimesh(nonequimesh), Ms_field(Ms_field.dims(3) == 1 ? af::tile(Ms_field, 1, 1, 1, 3) : Ms_field), m(m), verbose(verbose), mute_warning(mute_warning)
{
    if(Ms_field.dims(3) == 3){printf("%s State: You are using legacy dimension [nx, ny, nz, 3] for Ms, please now use scalar field dimensions [nx, ny, nz, 1].\n", Warning());}
...
double State::meani(const int i)
{
    ...
    {
        if (n_cells_ == 0)
            printf("%s State::meani: n_cells_ is empty and will be divieded by 0!\n", red("Warning:").c_str());

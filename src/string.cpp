...
void String::lin_interpolate()
{
    ...
    for (int i = 0; i < n_interp; i++)
    {
        ...
        {
            //std::cout << "Warning: x_interp[j=" << j << "] exceedes x range, y_interp[j] value set to y[j]" << std::endl;
            printf("Warning: x_interp[j=%d] exceedes x range, y_interp[j] value set to y[j]\n", j);
...
void write_plotfile(const std::string filepath)
{
    ...
    stream << "set key left top Left" << std::endl
           << std::endl;
    ...
    stream << "p 'E_last_step.dat' u 1:3 w l t 'Last iteration', 'E_max_lowest.dat' u 1:3 w l t 'Lowest barrier'" << std::endl;
    stream.close();
}
...
double String::run(const std::string filepath, const double string_abort_rel_diff, const double string_abort_abs_diff, const int string_steps, const bool verbose)
{
    ...
    for (int i = 0; i < string_steps; i++)
    {
        ...
        stream_E_barrier << "# max_lowest \t this->images[0].mesh.n0 \t this->images[0].mesh.dx" << std::endl;
    ...
    if (syscall == 0)
    {
        if (verbose)
        {
            ...
            //std::cout << "Plottet files using 'gnuplot plot_string_method.gpi'" << std::endl;
        }
    }
    else
    {

...
class AtomisticDmiFieldTest(unittest.TestCase):
  ...
  def test_atomistic_dmi_2_1_1_z_z(self):
    ...
    self.assertAlmostEqual(atom_dmi.E(state), 0)

    af_heff = atom_dmi.h(state)
    np_heff = af_heff.__array__()

    self.assertAlmostEqual(np_heff[0, 0, 0, 0], -D_atom/magnumaf.Constants.mu0/p )
    self.assertLess(math.fabs((np_heff[0, 0, 0, 0] - (-D_atom/magnumaf.Constants.mu0/p))/np_heff[0, 0, 0, 0]), 1e-15 )
    self.assertAlmostEqual(np_heff[0, 0, 0, 1], 0 )
    self.assertAlmostEqual(np_heff[0, 0, 0, 2], 0 )
    self.assertAlmostEqual(np_heff[1, 0, 0, 0], D_atom/magnumaf.Constants.mu0/p)
    self.assertLess(math.fabs((np_heff[1, 0, 0, 0] - D_atom/magnumaf.Constants.mu0/p)/np_heff[1, 0, 0, 0]), 1e-15)
  ...
  def test_atomistic_dmi_2_1_1_z_x(self):
    ...
    self.assertAlmostEqual(atom_dmi.E(state), - D_atom)
    ...
    self.assertAlmostEqual(np_heff[0, 0, 0, 1], 0 )
    self.assertAlmostEqual(np_heff[0, 0, 0, 2], D_atom/magnumaf.Constants.mu0/p )
    self.assertLess(math.fabs((np_heff[0, 0, 0, 2] - D_atom/magnumaf.Constants.mu0/p)/np_heff[0, 0, 0, 2]), 1e-15 )

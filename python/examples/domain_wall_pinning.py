...
if len(args.posargs) > 1:
    ...
    set_jump_K = False
    set_jump_Ms = False
print("set_jump_A, set_jump_K, set_jump_Ms:",
      set_jump_A, set_jump_K, set_jump_Ms)
...
H_analytic = Hc_analytic(soft_Aex, soft_ms, soft_K_uni,
                         hard_Aex, hard_ms, hard_K_uni)*Constants.mu0  # in [T]
...
field = Field(field_rate=2./Constants.mu0/100e-9,
              startField=0.95 * H_analytic/Constants.mu0)
stream = open(args.outdir + "m.dat", "w")
timer = time.time()
i = 0
nvti = 0
# NOTE marginal difference between 1e-3 and 1e-6
while (state.mean_m(wire_dir_val) < (1. - 1e-3)):
    if i % 300 == 0:
    ...
    printzee = af.mean(
        af.mean(af.mean(fields[0].H_in_Apm(state), dim=0), dim=1), dim=2)

    if i % 20 == 0:
        if H_analytic != 0.0:
            print("H[%analytic]", "{:6.4f}".format(printzee[0, 0, 0, wire_dir_val].scalar()*Constants.mu0 / H_analytic), "mean_m=", "{:5.4f}".format(state.mean_m(wire_dir_val)),
                  "field[Oe]=", "{:6.4f}".format(field.from_time(state.t)), "field[T]=", "{:6.4f}".format(printzee[0, 0, 0, wire_dir_val].scalar()*Constants.mu0), flush=True)
        else:
            print("mean_m=", "{:5.4f}".format(state.mean_m(wire_dir_val)), "field[Oe]=", "{:6.4f}".format(field.from_time(
                state.t)), "field[T]=", "{:6.4f}".format(printzee[0, 0, 0, wire_dir_val].scalar()*Constants.mu0), flush=True)
    stream.write("%e, %e, %e, %e, %e, %e\n" % (state.t, state.mean_m(0), state.mean_m(
        1), state.mean_m(2), field.from_time(state.t), printzee[0, 0, 0, wire_dir_val].scalar()))
    stream.flush()
    i = i + 1
...
print("H_analytic=", H_analytic, " [T]")
print("switched at Hext=", printzee[0, 0, 0, wire_dir_val].scalar(
)*Constants.mu0, " [T] with state.t=", state.t, " [s]")

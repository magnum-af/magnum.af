...
while (state.mean_mx() < (1. - 1e-3)):
    ...
    llg.step(state)
    outfile.write("%e, %e, %e, %e, %e\n" % (state.t, state.mean_mx(
    ), state.mean_my(), state.mean_mz(), field.from_time(state.t)))

outfile.close()

Hp_analytic_in_T = Hp_analytic_in_Apm(
    soft_Aex, soft_Ms, soft_Ku, hard_Aex, hard_Ms, hard_Ku) * maf.Constants.mu0

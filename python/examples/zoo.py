        ...
        while state.t < integration_time:
            ...
            for i in range(steps_per_iteration):
                ...
                if llg.accumulated_steps % write_vti_every == 0:
                    state.write_vti(args.outdir + "m_iD" + str(iD) + '_iH' +
                                    str(iH) + "step_" + str(llg.accumulated_steps))

        # Relaxing
        state.write_vti(args.outdir + 'm_relaxed_iD' +
                        str(iD) + '_iH' + str(iH))
        print('step', (iD * (dn + 1) + iH), '/', (dn + 1) **
              2 - 1, " took ", time.time() - timer, "[s]")

...
print("Init vortex in", time.time() - timer,
      "[s]")  # TODO use cython to speedup
# print(vortex.dims())
...
# region to evaluate mean values in reference layer
region_ref = disk(nx, ny, 1)
...
for iext_y in range(0, 6):
    ...
    stream.write(
        '#step={:d}, t[ns]={:1.6e}, mx={:1.6f}, my={:1.6f}, mz={:1.6f}\n')
    timer = time.time()
    write_vti_every = 100
    while state.t < 5e-9:
        llg.step(state)
        mx, my, mz = state.mean_m()
        print('step={:d}, t[ns]={:1.6e}, mx={:1.6f}, my={:1.6f}, mz={:1.6f}'.format(
            llg.accumulated_steps, state.t * 1e9, mx, my, mz))
        stream.write('{:d}\t{:1.6e}\t{:1.6f}\t{:1.6f}\t{:1.6f}\n'.format(
            llg.accumulated_steps, state.t * 1e9, mx, my, mz))
        if llg.accumulated_steps % write_vti_every == 0:
            state.write_vti(filepath + "m_relaxing_step_" +
                            str(llg.accumulated_steps))
            Util.write_vti(demag.H_in_Apm(state), dx, dy, dz,
                           filepath + "demag_step_" + str(llg.accumulated_steps))
    ...
    # evaluating mean_m only in circlic region under vortex
    mean_ref = Util.spacial_mean_in_region(reference_layer, region_ref)
    print('mean pinned       =', af.mean(
        af.mean(pinned_layer, dim=0), dim=1).to_ndarray()[0, 0, 0, :])
    print('mean ref          =', af.mean(
        af.mean(reference_layer, dim=0), dim=1).to_ndarray()[0, 0, 0, :])

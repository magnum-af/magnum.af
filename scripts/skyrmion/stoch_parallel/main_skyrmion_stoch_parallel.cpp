...
void Detector::add_data(double value) {
    if (data.size() < length) {
        ...
        if (gt == true && avg > threshold) {
            event = true;
        }
        if (gt == false && avg < threshold) {
            event = true;
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    // .magnum.af $PWD  ~/git/magnum.af/Data/skyrmion_stoch/E_barrier 1e-9 300 0
    // 0
    ...
    // << std::endl;
    //  std::vector<double> antimes;// appends annihilation time for each
    //  iteration to calculate mean
    ...
    while (detector.event == false) {
        ...
        // if(i < 100)  vti_writer_atom(state.m, state.mesh,
        // filepath+"/vti/dense_skyrm"+std::to_string(ID)+"_"+std::to_string(i));//state.t*pow(10,
        // 9) if(i < 100)  vti_writer_atom(state.m, state.mesh,
        // filepath+"/skyrm/dense_skyrm"+std::to_string(j)+"_"+std::to_string(i));//state.t*pow(10,
        // 9) if(i % (int)(1e-10/dt) == 0) vti_writer_atom(state.m, state.mesh,
        // filepath+"/skyrm/skyrm"+std::to_string(j)+"_"+std::to_string(i));//state.t*pow(10,
        // 9) if( ID < id_outp && i % (int)(1e-10/dt) == 0)
        // vti_writer_atom(state.m, state.mesh,
        // filepath+"/vti/skyrm"+std::to_string(ID)+"_"+std::to_string(i));//state.t*pow(10,
        // 9) if( ID < id_outp && i % (int)(1e-10/dt) == 0) std::cout << state.t
        // << " , i= " << i << ", mz    = "<< meani(state.m, 2) << std::endl;
        // if( ID < id_outp && i % (int)(1e-10/dt) == 0) std::cout << state.t <<
        // " , i= " << i << ", avg_mz= "<< detector.get_avg() << std::endl;
        if (images.size() < images_length && i % (int)(1e-11 / dt) == 0)
            images.push_back(state.m);
        if (images.size() > images_length && i % (int)(1e-11 / dt) == 0) {
            images.pop_front();
            images.push_back(state.m);
        }
    ...
    for (auto const& i : images) {
        vti_writer_atom(i, state.mesh,
                        filepath + "/vti/skyrm" + std::to_string(ID) + "_" +
                            std::to_string(count1)); // state.t*pow(10, 9)
        count1++;
    }
    // vti_writer_atom(state.m, state.mesh,
...
//  double unbiased_sample_variance = 0; // s^2= 1/(n-1) sum(y_i - y_mean)^2
//  from i = 1 to n for (double n : antimes){
...
//  std::endl; std::cout<<"unbiased_sample_sigma = "<< unbiased_sample_sigma <<
//  std::endl; ofs_antime.open(filepath +
//  "mean_antime"+std::to_string(ID)+".dat"); ofs_antime.precision(12);
//  ofs_antime<<"#mean_antime << unbiased_sample_variance <<
//  unbiased_sample_sigma<< antimes.size() "<< std::endl; ofs_antime<<
//  mean_antime <<  "\t" << unbiased_sample_variance <<"\t" <<
...
//        detector.add_data(i);
//        std::cout << i <<", "<< detector.data.size()<<", "<<
//        detector.data.front()<<" , "<< detector.data.back()<<" , "<<
//        detector.get_avg()<<std::endl;
//    }

#!/bin/bash
# Usage .sh absolute/path/to/write/output/ optional-GPU number main.cpp dmi Ku1
set -e
...
if [ -e $magafdir/bin/magnum.af-opencl ];then
    screen -d -S GPU$2 -m bash -c "$magafdir/bin/magnum.af-opencl $1 $2 $4 $5 > $1/cout.txt 2>&1"

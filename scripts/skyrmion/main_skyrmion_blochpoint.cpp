...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < string_steps; i++) {
        ...
        if (i > 25 && fabs(2 * (*max - string.E[0] - max_prev_step) / (*max - string.E[0] + max_prev_step)) <
        ...
        if (i > 25 && fabs(*max - string.E[0] - max_prev_step) < string_abort_abs_diff) {
            ...
            stream_steps << "#Exiting loop: Energy barrier difference smaller than" << string_abort_abs_diff
                         << std::endl;
            break;
        }
        ...
        stream_E_barrier << max_lowest << "\t" << nxy << "\t" << dx << "\t" << material.D << "\t" << material.Ku1

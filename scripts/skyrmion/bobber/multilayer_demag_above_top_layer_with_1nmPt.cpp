...
int main(int argc, char **argv)
{
    ...
    const uint32_t layers_above_top = 22; // adds 22 nm vacuum above top layer to evaluate demag. We evaluate at 21 nm, but need 22 layers to prevent prime number of layers conflicting with opencl fft.
    ...
    // Layer 13 (i.e. 1nm Pt interlayer) is strongly exchange coupled to layer 12 with very low Js.
    // js 1e-4 T, A = 10 A_rest, K=0; gros/ RKKY 
...
//    //const double Pt_D = 0;   // J/m^2
    const double Pt_Ms = 5e3; // A/m //NOTE: Small Ms highly decreases integration speed!!! Ms 1e3 or below causes smaller than 1e-15 s steps.

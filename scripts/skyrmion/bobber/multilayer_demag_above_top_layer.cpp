...
int main(int argc, char** argv) {
    ...
    vti_writer_micro(demag_init(af::span, af::span, 52, af::span), mesh,
                     filepath + "demag_init_21nm_above_top");

    write_ascii(demag_init, mesh, filepath + "demag_init.txt");
    write_ascii(demag_init(af::span, af::span, 52, af::span), mesh,
                filepath + "demag_init_21nm_above_top.txt");
    ...
    vti_writer_micro(demag_relaxed(af::span, af::span, 52, af::span), mesh,
                     filepath + "demag_relaxed_21nm_above_top");

    write_ascii(demag_relaxed, mesh, filepath + "demag_relaxed.txt");
    write_ascii(demag_relaxed(af::span, af::span, 52, af::span), mesh,

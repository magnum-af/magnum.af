...
int main(int argc, char** argv)
{
    ...
    while (state_1.t < 20e-9){
        if (state_1.steps % 100 == 0){
            state_1.write_vti(filepath + "m_step" + std::to_string(state_1.steps));

            std::cout << std::scientific << state_1.steps << ", t=" << state_1.t << ", mz=" <<  state_1.meani(2) << ", E=" << llg.E(state_1);
            for(unsigned i = 0; i < llg.llgterms.size(); i++) {
                std::cout << ", " << llgnames[i] << "=" << llg.llgterms[i]->E(state_1);
            }
            std::cout << std::endl;

            stream << std::scientific << state_1.steps << "\t" << state_1.t << "\t" <<  state_1.meani(2) << "\t" << llg.E(state_1);
            for(unsigned i = 0; i < llg.llgterms.size(); i++) {

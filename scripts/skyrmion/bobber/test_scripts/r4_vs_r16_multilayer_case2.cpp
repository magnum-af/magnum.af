...
af::array minit(const Mesh& mesh, const double r_in = 16.){
    ...
    for(uint32_t ix=0;ix<mesh.n0;ix++){
        for(uint32_t iy=0;iy<mesh.n1;iy++){
            ...
            if(r>mesh.n0/r_in) m(ix, iy, af::span, 2)=1.;
        }
    }

...
int main(int argc, char **argv)
{
    ...
    const uint32_t nxyin(argc > 6 ? std::stoi(argv[6]) : 100);
    const uint32_t nx = nxyin, ny = nxyin, nz = 32;
    std::cout << "nxy=" << nx << " " << ny << std::endl;
    ...
    m_case3(af::span, af::span, 13, 2) = 1.;
    m_case3(af::span, af::span, 14, 2) = 1.;
    m_case3(af::span, af::span, 15, 2) = 1.;
    m_case3(af::span, af::span, 16, 2) = 1.;
    ...
    if (!exists(filepath + "m_state_2_relaxed.vti"))
    {
        ...
        if (int_over_relax)
        {
            ...
            while (state_2.t < t_relax_state1_sec)
            { // 3ns not sufficient
    ...
    inputimages.push_back(state_2); // case3, higher E
    inputimages.push_back(state_1); // case2, lower E

...
int main(int argc, char** argv) {
    ...
    if (!exists(filepath + "m_relaxed.vti")) {
        ...
        std::cout << "mesh.dz after read =" << state_1.mesh.dz << std::endl;
        state_1.mesh = mesh;
        std::cout << "mesh.dz after overwrite =" << state_1.mesh.dz << std::endl;
    ...
    if (relax_per_step_else_continuous) {
        ...
        const double H_fixed_in_Oe = 155e-3 / constants::mu0;
        auto calc_Hext = [H_fixed_in_Oe, H_xz_plane_tilt_in_degree](State state) {
            af::array field = af::constant(0.0, state.mesh.n0, state.mesh.n1, state.mesh.n2, 3, f64);
            const double H_x = H_fixed_in_Oe * std::sin(2 * M_PI * H_xz_plane_tilt_in_degree / 360.);
            const double H_z = H_fixed_in_Oe * std::cos(2 * M_PI * H_xz_plane_tilt_in_degree / 360.);

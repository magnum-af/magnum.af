...
// TODO this file was used to see wether the skyrmion changes by employing
    ...
    for (int i = 0; i < string_steps; i++) {
        ...
        if (*max - string.E[0] < max_lowest) {
            ...
            std::cout << "Exiting loop: Energy barrier after 50step relaxation "
                         "becomes bigger "
                      << std::endl;
            stream_steps << "#Exiting loop: Energy barrier after 50step "
                            "relaxation becomes bigger "
                         << std::endl;
            break;
        }
        // std::cout<<"Test: fabs=
        ...
        std::cout << i << "\t" << *max - string.E[0] << "\t" << *max - string.E[-1] << "\t" << *max << "\t"
                  << fabs(2 * (*max - string.E[0] - max_prev_step) / (*max - string.E[0] + max_prev_step)) << std::endl;
        stream_steps << i << "\t" << *max - string.E[0] << "\t" << *max - string.E[-1] << "\t" << *max << "\t"

...
int main(int argc, char** argv)
{
    ...
    auto zee_func = [ A, B ] ( State state ) -> af::array {
        ...
        af::array zee = af::array(state.mesh.dims, f32);
        zee(af::span, af::span, af::span, 0)=constant(A * std::cos(phi), dim, f32);
        zee(af::span, af::span, af::span, 1)=constant(A * std::sin(phi), dim, f32);
        zee(af::span, af::span, af::span, 2)=constant(0.0              , dim, f32);
        //zee(af::span, af::span, af::span, 2)=constant( A * std::sin(phi) , state.mesh.n0, state.mesh.n1, state.mesh.n2, 1, f32);
        return  zee;
    };
    ...
    float A_ex = 15e-12;// [J/m]
    float Ku1   = 1.3e-3/z;      // [J/m^3] // Ku1 = K_total - K_shape = Hk*Js/2/mu0 + Js^2/2/mu0 = | [Hk and Js in Tesla] | = ((0.1*1.58)/2/(4*pi*1e-7) + (1.58)^2/(2)/(4*pi*1e-7)) = 1.056e6

    float Ku1_stress = 1400; //TODO
    ...
    minimizer.llgterms_.push_back( LlgTerm (new ExchangeField(A_ex)));
    minimizer.llgterms_.push_back( LlgTerm (new UniaxialAnisotropyField(Ku1)));
    minimizer.llgterms_.push_back( LlgTerm (new UniaxialAnisotropyField(Ku1_stress, {1, 0, 0})));

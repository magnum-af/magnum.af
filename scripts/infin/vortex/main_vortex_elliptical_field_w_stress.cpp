...
int main(int argc, char** argv) {
    ...
    auto zee_func = [steps_full_rotation, A, B](State state) -> af::array {
        double phi =
            2. * M_PI * (double)state.steps / (double)steps_full_rotation;
        array zee =
            constant(0.0, state.mesh.n0, state.mesh.n1, state.mesh.n2, 3, f64);
        zee(span, span, span, 0) =
            constant(A * std::cos(phi), state.mesh.n0, state.mesh.n1,
                     state.mesh.n2, 1, f64);
        zee(span, span, span, 1) =
            constant(B * std::sin(phi), state.mesh.n0, state.mesh.n1,
                     state.mesh.n2, 1, f64);
        zee(span, span, span, 2) =
            constant(A * std::sin(phi), state.mesh.n0, state.mesh.n1,
                     state.mesh.n2, 1, f64);
        return zee;
    };
    ...
    state.Ms =
        1.75 /
        constants::mu0;   //[J/T/m^3] == [Joule/Tesla/meter^3] = 1.75 T/mu_0
    material.A = 1.5e-11; //[J/m]
    std::cout << "A=" << A << "B= " << B
              << "steps_full_rotation=" << steps_full_rotation << std::endl;
    ...
    for (int i = 0; i <= steps_full_rotation; i++) {
        ...
        if (state.steps % 2000 == 0) {
            vti_writer_micro(state.m, mesh,
                             filepath + "m_rotation_" +
                                 std::to_string(state.steps));
        }
        state.steps++;
    }
    stream.close();
    std::cout << "time full rotation [af-s]: " << af::timer::stop(t_rot)
              << std::endl;
    return 0;
}

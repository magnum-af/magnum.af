...
int main(int argc, char** argv) {
    ...
    auto calc_hz = [nx, ny, x, y, filepath, &i_callback, idz_vortex](double dz) -> double {
        ...
        stream << z_spacing[1] << ", "
               << afvalue(h(nx / 2, ny / 2, 5, 0)) << ", "
               << afvalue(h(nx / 2, ny / 2, 5, 1)) << ", "
        ...
        //             0));
        return afvalue(
            af::mean(af::mean(af::mean(af::mean(h(af::span, af::span, fl_seq, af::span) *
                                  af::tile(m_free_geom_for_meandemag, 1, 1, idz_vortex, 3) ,
                              3),
                              2),
                          1),
    ...
    auto result =
        ni.run(X0(10.001e-9), Precision(1e-8), EpsilonFactor(1e-10), Imax(100));

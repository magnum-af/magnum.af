...
int main(int argc, char** argv) {
    ...
    const double t_full_rotation =
        double(argc > 5 ? std::stod(argv[5]) : (double)(800e-9));
    ...
    std::cout << "A=" << A << "B= " << B
              << "t_full_rotation=" << t_full_rotation << std::endl;
    ...
    state.calc_mean_m(stream, n_cells,
                      Llg.llgterms[Llg.llgterms.size() - 1]->h(state)(
                          0, 0, 0, af::span)); // To checkback H_zee for init

...
int main(int argc, char **argv)
{
    ...
    for (int nz = 2; nz < 22; nz++)
    {
        ...
        ////vti_writer_micro(state.m, mesh, filepath + "minit_nonnormalized" + std::to_string(nz));
        //state.m = renormalize_handle_zero_values(state.m);
        //vti_writer_micro(state.m, mesh, filepath + "minit_renorm" + std::to_string(nz));
        ...
        //vti_writer_micro(state.m, mesh, filepath + "m_hysteresis_" + std::to_string(nz));
        state.write_vti(filepath + "m_relaxed" + std::to_string(nz));
        std::cout << nz << "\t" << state.meani(0) << "\t" << state.meani(1) << "\t" << state.meani(2) << "\t" << afvalue(demag->h(state)(nx/2, ny/2, 0, 0)) * constants::mu0 << "\t" << std::endl;
        stream    << nz << "\t" << state.meani(0) << "\t" << state.meani(1) << "\t" << state.meani(2) << "\t" << afvalue(demag->h(state)(nx/2, ny/2, 0, 0)) * constants::mu0 << "\t" << std::endl;
    }
    stream.close();
    return 0;
}

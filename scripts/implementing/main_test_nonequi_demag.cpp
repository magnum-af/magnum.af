...
int main(int argc, char** argv)
{
    ...
    if (run_test == 0 || run_test == 1)
    {
        ...
        af::array m_nonequi = af::constant(0, nx, ny, nz_nonequi, 3, f32);

        m_nonequi(af::seq(1, af::end-1), af::span, af::span, 0) = af::constant(1.0, nx-2, ny, nz_nonequi, 1, f32);
        m_nonequi(0, af::span, af::span, 1 ) = af::constant(1.0, 1, ny, nz_nonequi, 1, f32);
        m_nonequi(-1, af::span, af::span, 1) = af::constant(1.0, 1, ny, nz_nonequi, 1, f32);

        State state_nonequi(mesh_nonequi, material, m_nonequi);

        af::array m_full = af::constant(0, nx, ny, nz, 3, f32);
        m_full(af::seq(1, af::end-1), af::span, 0, 0) = af::constant(1.0, nx-2, ny, 1, 1, f32);
        m_full( 0, af::span, 0, 1) = af::constant(1.0, 1, ny, 1, 1, f32);
        m_full(-1, af::span, 0, 1) = af::constant(1.0, 1, ny, 1, 1, f32);
    ...
    if (run_test == 0 || run_test == 2)
    {
        ...
        //test
        //rel_diff_lt_precision(af::constant(1., 1, f32), af::constant(1.1, 1, f32), 0.1);
        //rel_diff_lt_precision(af::constant(1., 1, f32), af::constant(1.1, 1, f32), 0.099);
        //rel_diff_lt_precision(af::constant(1., 1, f32), af::constant(1.1, 1, f32), 0.09);
        //rel_diff_lt_precision(af::constant(-1., 1, f32), af::constant(-1.1, 1, f32), 0.1);
        //rel_diff_lt_precision(af::constant(1., 1, f32), af::constant(1.1, 1, f32), 0.1);
        //rel_diff_lt_precision(af::constant(10., 1, f32), af::constant(10.1, 1, f32), 0.1);
    }
    ...
    if (run_test == 0 || run_test == 3)
    {
        ...
        //float reldiff = rel_diff_upperbound(demag.h(state_full), nonequi_demag.h(state_nonequi), false, 1e0, 0.1, 0.9);
        //float absdiff = abs_diff_upperbound(demag.h(state_full), nonequi_demag.h(state_nonequi), false, 1e0, 0.1, 0.9);

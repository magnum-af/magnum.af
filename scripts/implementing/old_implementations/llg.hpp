...
class LLG {
  public:
    LLG(State, std::vector<std::unique_ptr<LLGTerm>>);
    // Field Term Contributions
    std::vector<std::unique_ptr<LLGTerm>> Fieldterms;
    // Class Objects
    State state0;
    ...
    // array DP45(array m, double dt, double& rk_abs_error);// Dormand-Prince
    // with stepsize control

    // Bogacki-Shampine with stepsize control
    array BS45(const array&, const double dt, double& err);
    array BS45de(const array&, const double dt,
                 double& err); // Double error estimation

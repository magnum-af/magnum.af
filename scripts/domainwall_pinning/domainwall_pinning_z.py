...
#temp_workaround.py_vti_writer_micro(sys.argv[1] +"state.micro_A_field")
#temp_workaround = State(mesh, material, state.micro_Ms_field)
#temp_workaround.py_vti_writer_micro(sys.argv[1] +"state.micro_Ms_field")
#temp_workaround = State(mesh, material, state.micro_Ku1_field)
#temp_workaround.py_vti_writer_micro(sys.argv[1] +"state.micro_Ku1_field")
...
#print("m", state.m)
#print(state.t, state.meanxyz(0), state.meanxyz(1), state.meanxyz(2), fastenup * state.t/50e-9/Constants.mu0, printzee[0,0,0,0].scalar())
#while i < 10:

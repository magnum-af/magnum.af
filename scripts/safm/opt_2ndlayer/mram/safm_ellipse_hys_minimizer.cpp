...
int main(int argc, char **argv)
{
    ...
    auto zee_func = [h_demag_safm, hzee_max, steps_full_hysteresis, hext_angle](State state) -> af::array {
        ...
        {
            field_Tesla = 0;
            std::cout << "WARNING ZEE time out of range, setting external field to zero" << std::endl;
        }
    ...
    for (unsigned i = 0; i < steps_full_hysteresis * 5. / 4.; i++)
    {
        ...
        state.steps++;
        std::cout << "i=" << i << ", mean=" << state.meani(im_free_layer) << ", h=" << constants::mu0 * afvalue(minimizer.llgterms_.back()->h(state)(0, 0, 0, im_free_layer)) << std::endl;
    }
    stream.close();

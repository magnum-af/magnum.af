...
int main(int argc, char** argv) {
    ...
    auto zee_func = [h_demag_safm, hzee_max, steps_full_hysteresis,
                     hext_angle](State state) -> af::array {
        double field_Tesla;
    ...
    State state(mesh, Ms,
                mesh.ellipse({0.08748866352, 0, -1}, true)); // 5 degree tilted
    ...
    minimizer.llgterms_.push_back(
        LlgTerm(new UniaxialAnisotropyField(1.5 * Ms, {0, 0, 1})));
    ...
    for (unsigned i = 0; i < steps_full_hysteresis * 5. / 4.; i++) {
        ...
        state.calc_mean_m_steps(
            stream, constants::mu0 *
                        minimizer.llgterms_[minimizer.llgterms_.size() - 1]->h(
                            state)(0, 0, 0, af::span));
        if (state.steps % 10 == 0) {
            vti_writer_micro(
                state.m, mesh,

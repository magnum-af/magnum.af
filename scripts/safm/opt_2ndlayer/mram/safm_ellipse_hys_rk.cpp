...
int main(int argc, char** argv) {
    ...
    auto aniso = LlgTerm(
        new UniaxialAnisotropyField(1.5 * Ms, {0, 0, 1})); // TODO: value
    ...
    while (state.t < 5 * hzee_max / rate) {
        if (state.steps % 100 == 0)
            std::cout << "%=" << state.t / integr_time * 4. / 5.
                      ...
                      << ", <m>=" << state.meani(im_free_layer) << ", hzee="
                      << constants::mu0 * afvalue(llg.llgterms.back()->h(state)(
                                              0, 0, 0, im_free_layer))
                      << std::endl;
        llg.step(state);
        state.calc_mean_m_steps(
            stream, constants::mu0 * afvalue(llg.llgterms.back()->h(state)(
                                         0, 0, 0, im_free_layer)));

...
int main(int argc, char** argv)
{
    ...
    ////Generating Objects
    //These are the Vortex parameters for CoFeB:
    double Ms    = 1.393e6;//[J/T/m^3] == [Joule/Tesla/meter^3] = 1.75 T/mu_0
    double A     = 1.5e-11;//[J/m]
    //Paper [1] says about Ms=1.8T for bulk CoFeB
    //[1] "Ferromagnetic resonance and damping properties of CoFeB thin films as free layers inMgO-based magnetic tunnel junctions"
    ...
    while (state.t < 5* hzee_max/rate){
        if (state.steps % 100 == 0) std::cout << "%=" << state.t/integr_time * 4./5. << ", i=" << state.steps << ", t=" << state.t << ", <m>=" << state.meani(im_free_layer) <<  ", hzee=" << constants::mu0 * afvalue(llg.llgterms.back()->h(state)(0, 0, 0, im_free_layer)) << std::endl;
        llg.step(state);
        state.calc_mean_m_steps(stream, constants::mu0 * afvalue(llg.llgterms.back()->h(state)(0, 0, 0, im_free_layer)));

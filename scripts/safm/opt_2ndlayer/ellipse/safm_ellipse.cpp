...
int main(int argc, char** argv) {
    ...
    auto calc_hz = [nx, ny, x, y, filepath, &i_callback](double dz) -> double {
        ...
        double Ms = 1.393e6; // Vortex val//[J/T/m^3] == [Joule/Tesla/meter^3]
                             // = 1.75 T/mu_0
        ...
        stream.close();
        i_callback++;
        return afvalue(af::mean(
            af::mean(h(af::span, af::span, index_free_layer, 2), 1), 0));
    };
    ...
    auto result =
        ni.run(X0(8e-9), Precision(1e-8), EpsilonFactor(1e-10), Imax(100));

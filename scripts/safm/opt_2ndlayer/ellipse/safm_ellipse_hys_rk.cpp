...
int main(int argc, char **argv)
{
    ...
    LlgTerms llgterms = {demag, exch, zee};
    ...
    while (state.t < 5 * hzee_max / rate)
    {
        if (state.steps % 100 == 0)
            std::cout << "%=" << state.t / integr_time * 4. / 5. << ", i=" << state.steps << ", t=" << state.t << ", <m>=" << state.meani(0) << ", hzee=" << constants::mu0 * afvalue(llg.llgterms.back()->h(state)(0, 0, 0, 0)) << std::endl;
        llg.step(state);

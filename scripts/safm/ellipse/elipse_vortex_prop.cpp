...
{
    af::array m_mean = af::mean(af::mean(af::mean(m, 0), 1), 2);
    stream << std::fixed << factor * afvalue(m_mean(0, 0, 0, 0)) << "\t" << std::fixed << factor * afvalue(m_mean(0, 0, 0, 1)) << "\t" << std::fixed << factor * afvalue(m_mean(0, 0, 0, 2));
}
...
int main(int argc, char **argv)
{
    ...
    //TODO//auto aniso = LlgTerm (new NonequiUniaxialAnisotropyField(Ku1_field, std::array<double, 3>{0, 0, 1}));

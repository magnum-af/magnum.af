...
int main(int argc, char** argv) {
    ...
    // TODO vals?
    const double x = 5000.e-9, y = 1000e-9, z = 4.e-9;
    ...
    state.mesh = Mesh(nx, ny, nz, x / nx, y / ny,
                      z / nz); // TODO necessary for external field?
    // vtr_writer(state.m, Mesh(nx, ny, nz, x/nx, y/ny, 0) , z_spacing, filepath
    // + "m_init");
    ...
    // TODO//auto aniso = LlgTerm (new NonequiUniaxialAnisotropyField(Ku1_field,
    // std::array<double, 3>{0, 0, 1})); LlgTerms llgterms = {demag, exch,
    // aniso};
    ...
    if (int_over_min == "int") {
        ...
        }
        {
            ...
            auto zee_func_llg = [hzee_max, rate,
                ...
                // else if(state.t < 4*hzee_max/rate) field_Tesla = rate*state.t
                // - 4*hzee_max;
                else {
                    field_Tesla = rate * state.t - 4 * hzee_max;

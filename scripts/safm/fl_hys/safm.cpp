...
void print_m(const af::array& m, std::ostream& stream = std::cout) {
    af::array mean_m = af::mean(af::mean(af::mean(m, 0), 1), 2);
    stream << std::fixed << util::afvalue_as_f64(mean_m(0, 0, 0, 0)) << "\t" << std::fixed << util::afvalue_as_f64(mean_m(0, 0, 0, 1)) << "\t"
           << std::fixed << util::afvalue_as_f64(mean_m(0, 0, 0, 2));
}
...
int main(int argc, char** argv) {
    ...
    af::array Ku1_field = af::constant(0, mesh::dims_v(mesh), f64);
    ...
    auto aniso = uptr_FieldTerm(new NonequiUniaxialAnisotropyField(

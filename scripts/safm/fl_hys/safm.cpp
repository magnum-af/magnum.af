...
void print_m(const af::array& m, std::ostream& stream = std::cout){
    af::array m_mean = af::mean(af::mean(af::mean(m, 0), 1), 2);
    stream << std::fixed << afvalue(m_mean(0, 0, 0, 0)) << "\t" << std::fixed << afvalue(m_mean(0, 0, 0, 1)) << "\t" << std::fixed << afvalue(m_mean(0, 0, 0, 2));
}
...
int main(int argc, char** argv)
{
    ...
    //const float inttime = (argc > 6 ? std::stod(argv[6]): 10e-9); //[s]
    ...
    auto aniso = LlgTerm (new UniaxialAnisotropyField(Ku1_field, std::array<float, 3>{0, 0, 1}));//TODO energy is wrong is its not nonequi!
    ...
    if(int_over_min == "int")
    {
        ...
        {
            //float rate = 2e9 ; //[T/s]
            float int_time_per_quater = 10e-11;
            float rate = hzee_max/int_time_per_quater; //[T/s]
            std::cout << "hzee_max= " << hzee_max << ", rate=" << rate << ", int_time_per_quater=" << int_time_per_quater << std::endl;
    ...
    else
    {
        ...
        while (state.t < 4* hzee_max/rate){
            ...
            print_m(state.m, stream);
            stream << "\t" << afvalue(h_zee(0, 0, 0, 0)) << ", "  << afvalue(h_zee(0, 0, 0, 1)) << ", " << afvalue(h_zee(0, 0, 0, 2)) << std::endl;
            if(state.steps % 1 == 0){vtr_writer(state.m, Mesh(nx, ny, nz, x/nx, y/ny, 0) , z_spacing, filepath + "m_minimzed" + std::to_string(state.steps));}
            state.steps++;
            state.t+=1.;
        }
    }

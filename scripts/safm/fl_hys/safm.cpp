...
void print_m(const af::array& m, std::ostream& stream = std::cout) {
    af::array m_mean = af::mean(af::mean(af::mean(m, 0), 1), 2);
    stream << std::fixed << afvalue(m_mean(0, 0, 0, 0)) << "\t" << std::fixed
           << afvalue(m_mean(0, 0, 0, 1)) << "\t" << std::fixed
           << afvalue(m_mean(0, 0, 0, 2));
}
...
int main(int argc, char** argv) {
    ...
    // Generating Objects
    std::vector<double> z_spacing = {z / 4., 1e-10, 9.84825e-10, z / 4.,
                                     z / 4.};
    ...
    Ku1_field(af::span, af::span, 4, af::span) =
    ...
    auto aniso = LlgTerm(new UniaxialAnisotropyField(
        Ku1_field, std::array<double, 3>{
                       0, 0, 1})); // TODO energy is wrong is its not nonequi!
    LlgTerms llgterms = {demag, exch, aniso};
    timer.print_stage("setup ");

    af::array h = demag->h(state);
    vtr_writer(h, Mesh(nx, ny, nz, x / nx, y / ny, 0), z_spacing,
               filepath + "h");
    ...
    if (int_over_min == "int") {
        ...
        }
        {
            ...
            std::cout << "hzee_max= " << hzee_max << ", rate=" << rate
                      << ", int_time_per_quater=" << int_time_per_quater
                      << std::endl;
    ...
    // minimize
        ...
        while (state.t < 4 * hzee_max / rate) {
            ...
            if (state.steps % 1 == 0) {
                vtr_writer(
                    state.m, Mesh(nx, ny, nz, x / nx, y / ny, 0), z_spacing,

...
int main(int argc, char** argv)
{
  ...
  const int nx = 30, ny=30 ,nz=1;//nz=5 -> lz=(5-1)*dx
  ...
  std::cout<<"J_atom="<<param.J_atom<<std::endl;
...
//    
//    if(i>25 && fabs(2*(*max-string.E[0]-max_prev_step)/(*max-string.E[0]+max_prev_step))<1e-8){
//      std::cout   <<      "Exiting loop: Energy barrier relative difference smaller than 1e-8"<<std::endl;
//      stream_steps<<     "#Exiting loop: Energy barrier relative difference smaller than 1e-8"<<std::endl;
//      break;
//    }

...
TEST(NonEquiDemag, NfftSymmetryTest) {
    ...
    NonEquiDemagField nedemag = NonEquiDemagField(nemesh, true, false, 1);
    for (int i_source = 0; i_source < nemesh.nz; i_source++ ){
        for (int i_target = 0; i_target < nemesh.nz; i_target++ ){
            if (i_source >= i_target){
                const int zindex = i_target + nemesh.nz * i_source;
                const int zindex_swapped = i_source + nemesh.nz * i_target;
                EXPECT_NEAR(max_abs_diff(nedemag.Nfft(af::span, af::span, zindex, af::span), nedemag.Nfft(af::span, af::span, zindex_swapped, af::span)), 0, 1e-23);
            }
        }
    }
}
...
TEST(NonEquiDemag, SP4LayerRandomMagnetizationHeffTest) {
    ...
    EXPECT_NEAR(mean_abs_diff(constants::mu0 * demag_ed.h(state_ed)(af::span, af::span, 0, af::span), constants::mu0 * demag_ne.h(state_ne)(af::span, af::span, 0, af::span)), 0, 1e-8);
}
